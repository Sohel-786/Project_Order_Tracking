...
/**
 * Zero-based index of a dialog in the open stack (for layered z-index).
 */
export function getDialogStackIndex(closeFn: () => void): number {
  const index = dialogStack.findIndex((e) => e.close === closeFn);
  return index >= 0 ? index : Math.max(0, dialogStack.length - 1);
}
