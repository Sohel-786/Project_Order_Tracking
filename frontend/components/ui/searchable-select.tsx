...
export interface SearchableSelectProps {
  ...
  /** Optional callback to open an add dialog */
  onAdd?: () => void;
...
export function SearchableSelect({
  ...
  useEffect(() => {
    ...
    const updatePos = () => {
      ...
      const maxH = Math.max(240, Math.min(420, viewportH - rect.bottom - 16));
      setDropdownStyle({
        position: "fixed",
        top: rect.bottom + 6,
        left: rect.left,
  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      {isOpen && mounted && typeof document !== "undefined" && dropdownStyle != null && createPortal(
        <div
          ref={dropdownRef}
          style={dropdownStyle}
          className="rounded-md border border-secondary-200 dark:border-border bg-white dark:bg-card shadow-lg overflow-hidden"
        >
          <div className="border-b border-secondary-200 dark:border-border p-2 bg-secondary-50 dark:bg-secondary-900/50 flex flex-col gap-2">
            ...
            {onAdd && (
              <button
                ...
                className="h-9 w-full px-3 bg-primary-600 hover:bg-primary-700 text-white rounded-md text-[11px] font-black uppercase tracking-widest shadow-sm shadow-primary-900/20 active:scale-95 transition-all flex items-center justify-center"
              >
                {addLabel}
              </button>
            )}
          ...
          <ul
            ...
            style={{ maxHeight: dropdownStyle.maxHeight ? Number(dropdownStyle.maxHeight) - 64 : 360 }}
            role="listbox"
